...
def write(path, px, scale=1):
    """Writes RGB, or RGBA when any pixel carries a fourth channel."""
    h, w = len(px), len(px[0])
    rgba = any(len(c) > 3 for row in px for c in row)
    n, ctype = (4, 6) if rgba else (3, 2)
    buf = b''
    for row in px:
        cells = [bytes(tuple(c) + ((255,) if len(c) == 3 else ())) if rgba else bytes(c[:3]) for c in row]
        line = b''.join(cell * scale for cell in cells)
        buf += (b'\x00' + line) * scale
    png = b'\x89PNG\r\n\x1a\n' + _chunk(b'IHDR', struct.pack('>IIBBBBB', w * scale, h * scale, 8, ctype, 0, 0, 0))
