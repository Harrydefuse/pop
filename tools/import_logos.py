...
def main(src_dir, out_path, everywhere=()):
    ...
    for f in sorted(src.iterdir()):
        if f.suffix.lower() not in ('.png', '.jpg', '.jpeg', '.webp'):
            continue
        im = square(knockout(Image.open(f), f.stem in everywhere))
        im = im.resize((SIZE, SIZE), Image.LANCZOS)
        buf = io.BytesIO()
        im.save(buf, 'PNG', optimize=True)
        b64 = base64.b64encode(buf.getvalue()).decode()
        entries.append((f.stem, b64, len(buf.getvalue())))
        print(f'{f.name:>24} -> {f.stem:<14} {len(buf.getvalue()) / 1024:6.1f} kB')

    if not entries:
        raise SystemExit(f'nothing to import from {src}')

    body = ',\n'.join(f"  {k}: '{b}'" for k, b, _ in entries)
    out = (
