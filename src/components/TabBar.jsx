...
export default function TabBar({ tab, setTab, badges = {} }) {
  return (
    <nav className="relative z-20 border-t border-line bg-[#0d0918]/95 backdrop-blur grid grid-cols-5">
      {TABS.map((t) => {
        ...
        return (
          <button
            ...
            className="relative py-2.5 flex flex-col items-center gap-1.5 transition-colors"
          ...
          >
            ...
            {badges[t.key] > 0 && (
              <span className="absolute top-1.5 right-[26%] w-1.5 h-1.5 bg-danger" style={{ boxShadow: '0 0 8px var(--color-danger)' }} />
            )}
          </button>
        )
      })}
